...
/*
    ...
    [데이터 모델링]
    - 요구 분석 기반으로 수집한 데이터 > 분석 > 저장 구조 생성 > 도식화 > ERD
    - 데이터를 저장하기 위해 데이터 구조를 설계하는 작업
    - DBMS의 종류를 결정하지 않는다.
    - 개념 + 논리 + 물리
    - 개념 > 아주 간단히 표현한 설계도 > 테이블 이름과 관계 정도만 기술
    ...
    2. Entity, 엔티티
    - 다른 엔티티와 분류(구분)될 수 있고, 다른 엔티티에 대해 정해진 관계를 맺을 수 있는 데이터 단위
    - 테이블
    - 릴레이션 = 객체 = 레코드 = 엔티티
        a. 학생 정보 관리
        ...
        b. 강의실 정보 관리
            - 정보 수집 : 강의실 호수, 크기, 인원수, 용도, ...
            - 강의실(강의실 호수, 크기, 인원수, 용도)
        
    3. Attribute, 속성
    - 엔티티 내의 세부 정보
    - 엔티티를 만들기 위해 모인 여러가지 정보
    - 컬럼
    ...
    모델링 작업 > ERD > 정규화 과정을 통해 안정성 높고, 작업하기 편한 ERD로 수정
    ...
    [이상 현상]
    1. 삽입 이상, Insertion Abormaly
        - 특정 테이블에 데이터를 삽입할 때 원하지 않는 데이터까지 같이 넣어야 하는 상황 (e.g. NULL)
    
    2. 갱신 이상, Update Anormaly
        - 동일한 데이터가 2개 이상의 테이블에 동시에 수정되는 경우
            > 그 중 1개는 수정했지만 다른 1개를 수정하지 못했을 때 두 데이터 간 이상 오류 발생
    ...
    [제 1 정규화, 1NF]
    - 모든 컬럼(속성)은 원자값을 가진다.
    - 여러 개의 분리 가능한 값을 1개의 컬럼 안에 넣지 말것
