...
select (count(tel) - count(case when tel like '010%' then 1 end)) as "010을 안쓰는 사람" from tblInsa;

select count(*) from tblInsa where tel not like '010%';
    
    
-- 5. city. 서울, 경기, 인천 외의 지역에 사는 인원수? -> 18명
select 
    count(case
...
select count(case
    when ssn like '__07%-2%' or ssn like '__08%-2%' or ssn like '__09%-2%' then 1
    end) as "직원 수"
from tblInsa;
